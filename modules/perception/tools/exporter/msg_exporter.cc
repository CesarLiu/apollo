...
#include "modules/perception/tools/exporter/msg_exporter.h"
...
bool MsgExporter::SaveImage(const unsigned char* color_image,
  ...
  for (size_t y = 0; y < height; y++) {
    for (size_t x = 0; x < width; x++) {
      out.at<cv::Vec3b>(y, x)[0] = image.at<cv::Vec3b>(y, x)[2];  // R -> B
      out.at<cv::Vec3b>(y, x)[2] = image.at<cv::Vec3b>(y, x)[0];  // B -> R
      out.at<cv::Vec3b>(y, x)[1] = image.at<cv::Vec3b>(y, x)[1];  // G -> G
    }
  }
  static char path[500];
...
bool MsgExporter::IsCamera(const std::string& channel) {
  const std::vector<std::string> strs = absl::StrSplit(channel, '/');
  return strs.size() > 1 && strs.back() == "image";
}
