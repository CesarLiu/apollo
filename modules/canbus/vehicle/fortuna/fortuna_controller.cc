...
#include "modules/canbus/vehicle/fortuna/fortuna_controller.h"
...
ErrorCode FortunaController::Init(
  ...
  /* Sender part - No Apollo vehicle control functions implemented for Fortuna yet
...
void FortunaController::Stop() {
  if (!is_initialized_) {
    AERROR << "FortunaController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "FortunaController stopped.";
  }
}
...
Chassis FortunaController::chassis() {
  ...
  // 8 
  //Fortuna: Set throttle via input (pedal pressure percentage) and not
  //via output(control command) for now
  if (chassis_detail.has_gas() && chassis_detail.gas().has_throttle_input()) {
    chassis_.set_throttle_percentage(
        static_cast<float>(chassis_detail.gas().throttle_input()));
  } else {
    chassis_.set_throttle_percentage(0);
  }
  // 9
  //Fortuna: As above, set brake via input
  if (chassis_detail.has_brake() && chassis_detail.brake().has_brake_input()) {
    chassis_.set_brake_percentage(
        static_cast<float>(chassis_detail.brake().brake_input()));
  } else {
  ...
  if(chassis_detail.has_fortuna() && chassis_detail.fortuna().has_steering() &&
    ...
    chassis_detail.fortuna().steering().has_steering_wheel_angle_sign()) {
      ...
      const double steering_angle = steering_wheel_angle / 
        steering_angle_range_rad_to_steering_wheel_angle_range_deg_gain;
      const double steering_angle_max = vehicle_params_.max_steer_angle(); // TODO MEASURE!!! 
      const double steering_angle_percentage = steering_angle * 100.0 / steering_angle_max;
      
      chassis_.set_steering_percentage(
        static_cast<float>(steering_angle_percentage));
  } else {
...
ErrorCode FortunaController::EnableAutoMode() {
  
  //AINFO << "Fortuna currently supports only COMPLETE_MANUAL mode";
  //return ErrorCode::OK;
  //TODO Odys: check with Tobias which error code to return here
  //return ErrorCode::CANBUS_ERROR;
...
ErrorCode FortunaController::DisableAutoMode() {
  //AINFO << "Fortuna currently supports only COMPLETE_MANUAL mode";
  //return ErrorCode::OK;
...
ErrorCode FortunaController::EnableSteeringOnlyMode() {
  AINFO << "Fortuna currently supports only COMPLETE_MANUAL mode";
  return ErrorCode::OK;
  /*
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
ErrorCode FortunaController::EnableSpeedOnlyMode() {
  AINFO << "Fortuna currently supports only COMPLETE_MANUAL mode";
  return ErrorCode::OK;
  /*
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
void FortunaController::Gear(Chassis::GearPosition ref_gear_position) {
  // This case includes the Manual mode of Fortuna 
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
  ...
  AINFO << "Fortuna currently does not support changing gears";
...
// -> pedal
void FortunaController::Brake(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
  ...
  //A simple message in case Fortuna is used with not Manual mode
  AINFO << "Fortuna currently does not support braking";
  //brake_60_->set_pedal(pedal);
}

// drive with old acceleration
// gas:0.00~99.99 unit:%
void FortunaController::Throttle(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
  ...
  //A simple message in case Fortuna is used with not Manual mode
  AINFO << "Fortuna currently does not support throttle commands";
  //throttle_62_->set_pedal(pedal);
}

// drive with acceleration/deceleration
// acc:-7.0 ~ 5.0, unit:m/s^2
void FortunaController::Acceleration(double acc) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
  ...
  // None
  //A simple message in case Fortuna is used with not Manual mode
  AINFO << "Fortuna currently does not support acceleration commands";
}

// fortuna default, -470 ~ 470, left:+, right:-
// need to be compatible with control module, so reverse
// steering with old angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
void FortunaController::Steer(double angle) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
  ...
  //A simple message in case Fortuna is used with not Manual mode
  AINFO << "Fortuna currently does not support steering commands";
...
void FortunaController::Steer(double angle, double angle_spd) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
...
void FortunaController::SetEpbBreak(const ControlCommand &command) {
...
void FortunaController::ResetProtocol() {
...
bool FortunaController::CheckResponse(const int32_t flags, bool need_wait) {
...
void FortunaController::set_chassis_error_mask(const int32_t mask) {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  chassis_error_mask_ = mask;
}

int32_t FortunaController::chassis_error_mask() {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  return chassis_error_mask_;
}

Chassis::ErrorCode FortunaController::chassis_error_code() {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  return chassis_error_code_;
}

void FortunaController::set_chassis_error_code(
