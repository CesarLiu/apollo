...
void FrontObject2rpt0x32::Parse(const std::uint8_t* bytes, int32_t length,
    ...
    chassis_detail->mutable_front_object_2()->set_rel_velocity_x(rel_velocity_x(bytes, length));                           
    chassis_detail->mutable_front_object_2()->set_rel_pos_y(rel_pos_y(bytes, length));                           
    chassis_detail->mutable_front_object_2()->set_rel_pos_x(rel_pos_x(bytes, length));                           
    chassis_detail->mutable_front_object_2()->set_id(id(bytes, length));                           
    if(fused_state(bytes, length)){
        chassis_detail->mutable_front_object_2()->set_fused_state(Front_object_2_Fused_stateType_OBJECT_FUSED);                           
    }
    else{
        chassis_detail->mutable_front_object_2()->set_fused_state(Front_object_2_Fused_stateType_NOT_FUSED);
    }
