...
void RearObject1rpt0x41::Parse(const std::uint8_t* bytes, int32_t length,
    ...
    if(rear_tracked(bytes, length)){
        chassis_detail->mutable_rear_object_1()->set_rear_tracked(Rear_object_1_Rear_trackedType_OBJECT_MEASURED);                             
    }
    else{
        chassis_detail->mutable_rear_object_1()->set_rear_tracked(Rear_object_1_Rear_trackedType_OBJECT_TRACKED);                             
    }

    chassis_detail->mutable_rear_object_1()->set_rear_id(rear_id(bytes, length));
    chassis_detail->mutable_rear_object_1()->set_rear_pos_y(rear_pos_y(bytes, length));
    chassis_detail->mutable_rear_object_1()->set_rear_rel_velocity_x(rear_rel_velocity_x(bytes, length));
    chassis_detail->mutable_rear_object_1()->set_rear_pos_x(rear_pos_x(bytes, length));
}
