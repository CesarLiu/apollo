...
FrontObject4rpt0x34::FrontObject4rpt0x34() {}
...
int FrontObject4rpt0x34::checksum(const std::uint8_t* bytes, 
...
// ''}
// Relative velocity of the second object ahead on ego lane in direction of X.
double FrontObject4rpt0x34::rel_velocity_x(const std::uint8_t* bytes, 
...
// Position of the second object (object center) ahead on ego lane in
// direction of Y relative to the longitudinal axis of the ego vehicle.
double FrontObject4rpt0x34::rel_pos_y(const std::uint8_t* bytes, 
                             const int32_t length) const{
...
// 'type': 'double', 'order': 'intel', 'physical_unit': ''}
// Position of the second object (closest point) ahead on ego lane in direction
// of X relative to the rear axle of the ego vehicle. The distance is based on
// the current course of the ego vehicle. 
double FrontObject4rpt0x34::rel_pos_x(const std::uint8_t* bytes, 
                                 const int32_t length) const{
...
// ''}
// ID of FrontObject
int32_t FrontObject4rpt0x34::id(const std::uint8_t* bytes, 
                         const int32_t length) const{
...
bool FrontObject4rpt0x34::fused_state(const std::uint8_t* bytes, 
                             const int32_t length) const{
