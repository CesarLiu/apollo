...
bool FortunaVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Fortuna>(FLAGS_chassis_detail_topic);

  return true;
}

bool FortunaVehicleFactory::Start() {
...
void FortunaVehicleFactory::Stop() {
  can_sender_.Stop();
  can_receiver_.Stop();
  can_client_->Stop();
...
void FortunaVehicleFactory::UpdateCommand(
...
Chassis FortunaVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void FortunaVehicleFactory::PublishChassisDetail() {
...
FortunaVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Fortuna>>(
      new fortuna::FortunaController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Fortuna>>
FortunaVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Fortuna>>(
      new fortuna::FortunaMessageManager());
}
